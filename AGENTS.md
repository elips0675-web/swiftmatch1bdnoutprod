...
- **Java для Gradle:** Android Studio 2026 ставит JBR = Java 25, а Gradle 8.14.3 не стартует (major version 69). Нужен JDK 21 (zip Temurin из api.adoptium.net). **JDK развёрнут в `C:\jdk21\jdk-21.0.12.1+1`** (стабильный путь, НЕ в `%TEMP%\opencode` — Dr.Web агрессивно сканирует TEMP и даёт ложные срабатывания на официальный Temurin), `JAVA_HOME` на него.
- **Dr.Web whitelist (чтобы не ругался):** в GUI SpIDer Guard → Исключения добавить: `C:\jdk21`, `D:\swiftmatch1bddomadm\android\app\build`, `D:\swiftmatch1bddomadm\node_modules`, `D:\swiftmatch1bddomadm\server\node_modules`, `C:\Users\PC\.gradle`, `C:\Users\PC\AppData\Local\Android\Sdk`. Токсичные для Dr.Web `.js.jar`/`.json.jar` в `android\app\build\intermediates\compressed_assets` больше не создаются — `android.enableAppCompression=false` в `android\gradle.properties`.
...
## Admin Content: цепочка данных (DB → API → Админка → Приложение)

### Схема БД
Таблица `content_config` — **singleton** (одна строка, `id=1`). Все секции — JSON-колонки:
```
content_config (id=1):
  interests     JSON  — ["interest.sport","interest.music",...] (28 items)
  dating_goals  JSON  — ["goal.serious_relationship",...] (10 items)
  education     JSON  — ["secondary","higher",...] (8 items)
  banned_words  JSON  — ["спам","мошенничество",...] (109 items)
  cities        JSON  — ["Москва","Санкт-Петербург",...] (nullable)
```

### Три отдельных «banned words» системы
| Где | Источник | Используется |
|-----|----------|-------------|
| `content_config.banned_words` | **БД** (админка) | Чаты (`social.js`), хэнгауты (`hangouts.js`) через `banned-words.js` |
| `constants.ts: BANNED_WORDS` | **Хардкод** `["Хуй"]` | `profile.tsx`, `profile-edit.tsx` (фильтр интересов из отображения) |
| `groups.tsx: forbiddenWords` | **Хардкод** `["spam", "нецензурно"]` | Только фронт, только при создании группы |

**Важно:** изменение `content_config.banned_words` через админку НЕ влияет на фильтр интересов и названия групп.

### Цепочка чтения
```
БД content_config (id=1)
  → GET /api/content (публичный, index.js:202)  ← фронт читает ОТСЮДА
  → GET /api/admin/content (админ, content.js:25) ← НЕ используется фронтом
  → useContentConfig.ts (кэш + fallback на FORBIDDEN_WORDS_DEFAULT)
  → admin-content.tsx (useContentConfig → state → UI)
```

### Цепочка записи
```
admin-content.tsx: handleSave(section, items)
  → saveSection(section, items)  — stripPrefix убирает interest./goal./education. префиксы
  → PUT /api/admin/content/{section}  — body: { items: ["sport","music"] }
  → content.js: getContentConfig() читает ВСЕ колонки
  → config[section] = req.body.items  — заменяет ТОЛЬКО целевую секцию
  → INSERT ... ON DUPLICATE KEY UPDATE ${section} = ?
  → invalidateContentCache() → фронт перечитывает
```

### МОЖНО ЛИ сломать одну секцию при сохранении другой?
**Нет.** Каждый PUT затрагивает ТОЛЬКО одну колонку (`ON DUPLICATE KEY UPDATE ${section} = ?`).
Старые значения других колонок не перезаписываются.

### ЗАПРЕЩЕНО
- Не менять SQL-структуру `content_config` без миграции и同步 с `mysql_schema.sql`
- Не добавлять новые колонки через ALTER TABLE в `.js` файлах — только через `database/migrations/`
- Не трогать `getContentConfig()` в `content.js` — он используется и для GET, и для PUT (read-modify-write)
- Не удалять `invalidateContentCache()` после PUT — иначе фронт покажет stale данные
- Не хардкодить banned-words на фронте для чат-модерации — только через `content_config` в БД
- Не путать `GET /api/content` (публичный, читает фронт) с `GET /api/admin/content` (админ, не используется)
...
- Все 307 server + 81 frontend тестов проходят (388/388, 0 failures)
