...
9. **Анти-баблбл протокол (НЕМЕДЛЕННО, приоритет №1)** — категорически ЗАПРЕЩЕНО генерировать реплики-«намерения» без фактического вызова инструмента (типа текста «Doing edit / Doing git / сейчас сделаю X» без `tool_call`). Правила:
   - Любое решаемое действие (правка файла, чтение, git, grep) — выполняется ТОЛЬКО фактическим вызовом инструмента в **той же** реплике. Текст-намерение вместо вызова = дефект.
   - Если ты заметил, что повторил одно и то же намерение дважды БЕЗ вызова — немедленно потоком выполни фактический `tool_call`.
   - Никогда не «размышляй вслух» о том, что сейчас вызовешь инструмент, — саморазмышление и порождает эту петлю. Сразу вызывай.
   - Триггер самодиагностики: если в выводе появилось >2 реплик с намерением и без `tool_call` — остановись и в следующей реплике выполни только фактический вызов (без текста).
